...
$("#main").on("click", "button", function () {
  ...
  var startObj = $(this);
  var endObj = $(".bycar");
  $.fnInit(startObj, endObj).fnMove(moveImg); // fnFly(startObj,endObj,moveImg);
  ...
  var json = {
    ...
    count: 1 //再次点击时，先将cookie中的数据取出来（是一个数组）
    //将取出来的数组 先存入到 arr中 
  ...
  if (brr.length != 0) {
    ...
    arr = brr; //遍历数组arr  判断当前点击的商品是否在arr中存在，如果存在就将该商品的数量累加
...
function getCookie(key) {
  ...
  if (document.cookie) {
    var cookieInfo = document.cookie; //cookie中可能会包含一些 额外的数据，这些数据特点是由   分号和空格间隔的
    //所以 先将 分号和空格  替换掉   替换成  ;
...
$.extend({
  fnInit: function (startObj, endObj) {
    ...
    this.startPoint = {
      ...
      "y": startObj.offset().top //结束点坐标

    };
    this.endPoint = {
      "x": endObj.offset().left + endObj.width() / 2,
      "y": endObj.offset().top //最高点坐标

    };
    this.topPoint = {
      "x": this.endPoint.x - 100,
      "y": this.endPoint.y - 80 //根据三点坐标 
      //根据三点坐标确定抛物线的系数
