...
(function (undefined) {
  ...
  function configurePackageDir(pkgs, currentPackages, dir) {
    ...
    for (i = 0; pkgObj = currentPackages[i]; i++) {
      ...
      location = pkgObj.location; //Add dir to the path, but avoid paths that start with a slash
  ...
  function newContext(contextName) {
    ...
    function normalize(name, baseName) {
      var pkgName, pkgConfig; //Adjust any relative paths.
      ...
      if (name && name.charAt(0) === ".") {
        ...
        if (baseName) {
          ...
          trimDots(name); //Some use of packages may use a . path to reference the
      ...
      if (name) {
        if (prefix) {
          ...
          if (!url) {
            ...
            //messy to support.
            url = context.nameToUrl(name, null, parentModuleMap); //Store the URL mapping for later.
    ...
    function execManager(manager) {
      var i,
          ...
          cjsModule; //Call the callback to define the module, if necessary.
      ...
      if (cb && isFunction(cb)) {
        ...
        if (fullName) {
          ...
          cjsModule.exports !== defined[fullName]) {
            ...
            defined[fullName] = ret; //If this module needed full execution in a build
        ...
        ret = defined[fullName] = cb; //If this module needed full execution in a build
        //environment, mark that now.
      ...
      delete managerCallbacks[fullName]; //Allow instrumentation like the optimizer to know the order
    ...
    function callPlugin(pluginName, depManager) {
      var map = depManager.map,
          ...
          load; //No need to continue if the manager is already
      ...
      load = function (ret) {
        ...
        loaded[depManager.id] = true; //The loading of this plugin
      ...
      load.fromText = function (moduleName, text) {
        /*jslint evil: true */
        var hasInteractive = useInteractive; //Indicate a the module is in process of loading.

        loaded[moduleName] = false;
        context.scriptCount += 1; //Indicate this is not a "real" module, so do not track it
        //for builds, it does not map to a real file.

        context.fake[moduleName] = true; //Turn off interactive script matching for IE for any define
      ...
      if (fullName in defined) {
        ...
        plugin.load(name, makeRequire(map.parentMap, true, function (deps, cb) {
          var moduleDeps = [],
              i,
              dep,
              depMap; //Convert deps to full names and hold on to them
    ...
    function getManager(map, shouldQueue) {
      ...
      if (!manager) {
        ...
        specified[manager.id] = true; //Only track the manager/reuse it if this is a non-plugin
      ...
      if (prefix && !plugin) {
        prefixMap = makeModuleMap(prefix); //Clear out defined and urlFetched if the plugin was previously
        ...
        pluginManager.add(function (plugin) {
          ...
          var newMap = makeModuleMap(map.originalName, map.parentMap),
              normalizedManager = getManager(newMap, true); //Indicate this manager is a placeholder for the real,
    ...
    function main(inName, depArray, callback, relModuleMap) {
      ...
      if (fullName) {
        ...
        specified[id] = true;
        loaded[id] = true; //If module is jQuery set up delaying its dom ready listeners.
      ...
      manager.callback = callback; //Add the dependencies to the deps field, and register for callbacks
      //on the dependencies.

      for (i = 0; i < depArray.length; i++) {
        depArg = depArray[i]; //There could be cases like in IE, where a trailing comma will
        ...
        if (depArg) {
          ...
          depPrefix = depArg.prefix; //Fix the name in depArray to be just the name, since
          //that is how it will be called back later.

          depArray[i] = depName; //Fast path CommonJS standard dependencies.
          ...
          if (depName === "require") {
            ...
            if (fullName in needFullExec) {
              needFullExec[depName] = true; //Reset state so fully executed code will get
              //picked up correctly.
      ...
      if (!context.jQuery) {
        ...
        if ($) {
          ...
          if ("holdReady" in $ || "readyWait" in $) {
            context.jQuery = $; //Manually create a "jquery" module entry if not one already
    ...
    function findCycle(manager, traced) {
      ...
      traced[fullName] = true; //Trace through the dependencies.
    ...
    function checkLoaded() {
      ...
      expired = waitInterval && context.startTime + waitInterval < new Date().getTime(),
          ...
          manager,
          cycleManager,
          moduleDeps; //If there are items still in the paused queue processing wait.
      ...
      checkLoadedDepth = 0; //Check for DOM ready, and nothing is waiting across contexts.
    ...
    resume = function () {
      var manager, map, url, i, p, args, fullName; //Any defined modules in the global queue, intake them now.
      ...
      if (!config.priorityWait || isPriorityDone()) {
        while (context.paused.length) {
          p = context.paused;
          context.pausedCount += p.length; //Reset paused list
          ...
          for (i = 0; manager = p[i]; i++) {
            map = manager.map;
            url = map.url;
            fullName = map.fullName; //If the manager is for a plugin managed resource,
            ...
            if (map.prefix) {
              ...
              if (!urlFetched[url] && !loaded[fullName]) {
                (config.requireLoad || req.load)(context, fullName, url); //Mark the URL as fetched, but only if it is
    ...
    context = {
      ...
      configure: function (cfg) {
        var paths, prop, packages, pkgs, packagePaths, requireWait; //Make sure the baseUrl ends in a slash.
        ...
        pkgs = config.pkgs; //Mix in the config values, favoring the new values over
        //existing ones in context.config.

        mixin(config, cfg, true); //Adjust paths if necessary.
        ...
        if (cfg.priority) {
          ...
          requireWait = context.requireWait; //Allow tracing some require calls to allow the fetching
          //of the priority config.

          context.requireWait = false; //But first, call resume to register any defined modules that may
          //be in a data-main built file before the priority config
          //call.

          resume();

          context.require(cfg.priority); //Trigger a resume right away, for the case when
      ...
      require: function (deps, callback, relModuleMap) {
        ...
        if (typeof deps === "string") {
          ...
          moduleName = deps;
          relModuleMap = callback; //Normalize module name, if it contains . or ..
      ...
      nameToUrl: function (moduleName, ext, relModuleMap) {
        var paths,
            ...
            parentModule,
            url,
            config = context.config; //Normalize module name if have a base relative module name to work from.

        moduleName = normalize(moduleName, relModuleMap && relModuleMap.fullName); //If a colon is in the URL, it indicates a protocol is used and it is just
  ...
  req = requirejs = function (deps, callback) {
    ...
    var contextName = defContextName,
        context,
        config; // Determine if have config object in the call.
  ...
  req.load = function (context, moduleName, url) {
    ...
    context.scriptCount += 1;
    req.attach(url, context, moduleName); //If tracking a jQuery, then make sure its ready callbacks
  ...
  define = function (name, deps, callback) {
    var node, context; //Allow for anonymous functions
  ...
  req.onScriptLoad = function (evt) {
    ...
    if (evt.type === "load" || node && readyRegExp.test(node.readyState)) {
      ...
      contexts[contextName].completeLoad(moduleName); //Clean up script binding. Favor detachEvent because of IE9
  ...
  req.attach = function (url, context, moduleName, callback, type, fetchOnlyFunction) {
    ...
    if (isBrowser) {
      ...
      node.charset = "utf-8"; //Use async so Gecko does not block on executing the script if something
      ...
      node.src = url; //Fetch only means waiting to attach to DOM after loaded.
    ...
    for (globalI = scripts.length - 1; globalI > -1 && (script = scripts[globalI]); globalI--) {
      ...
      if (dataMain = script.getAttribute('data-main')) {
        if (!cfg.baseUrl) {
          ...
          src = dataMain.split('/');
          mainScript = src.pop();
          subPath = src.length ? src.join('/') + '/' : './'; //Set final config.

          cfg.baseUrl = subPath; //Strip off any trailing .js since dataMain is now
          //like a module name.
  ...
  req.resourcesReady = function (isReady) {
    var contexts, context, prop; //First, set the public variable indicating that resources are loading.
  ...
  req(cfg); //If modules are built into require.js, then need to make sure dependencies are
  //traced. Use a setTimeout in the browser world, to allow all the modules to register
  //themselves. In a non-browser env, assume that modules are not built into require.js,
  //which seems odd to do on the server.

  if (req.isAsync && typeof setTimeout !== "undefined") {
    ctx = s.contexts[cfg.context || defContextName]; //Indicate that the script that includes require() is still loading,
